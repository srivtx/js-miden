...
- "The `getNextLsn()` function uses an in-memory counter. In production, LSN must come from the database (e.g., `pg_current_wal_lsn()`)."
...
- **Offset Manipulation**: No authorization on offset updates. A malicious actor can skip events or force replays.
...
- **What's missing**: Exactly-once semantics discussion. While at-least-once is practical, students should understand the theoretical impossibility of exactly-once in distributed systems.
...
- Added `readChangesSince()` with `ORDER BY lsn ASC` for ordering guarantees.
