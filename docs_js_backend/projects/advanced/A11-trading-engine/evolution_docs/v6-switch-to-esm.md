# v6 — Switch to ESM (Trading Engine)
...
```typescript
// src/routes/orders.ts (CommonJS style)
...
## ESM Evolution in the Trading Engine
...
>
> You: "Because ESM is browser-native, and browsers don't know about `.ts`. TypeScript is a compile-time fiction. The runtime sees `.js`. Write what the runtime sees. In a trading engine, module resolution failures during market hours are not an option."

## The Next PAIN

ESM works. TypeScript compiles. Tests run. But your order book is still an in-memory Map. Your `.env` file has no database URL. You deploy to a cloud instance. The filesystem is ephemeral. Your orders vanish on every deploy.
