# v6 — Switch to ESM (Financial Ledger)
...
```typescript
// src/routes/transaction.routes.ts (CommonJS style)
...
import type { JournalEntry, Account } from '../types/transaction.types.js';
```
...
import { ledgerService } from '../services/ledger.service.js'; // ESM
...
## ESM Evolution in the Financial Ledger
...
>
> You: "Because ESM is browser-native, and browsers don't know about `.ts`. TypeScript is a compile-time fiction. The runtime sees `.js`. Write what the runtime sees. In a financial ledger, module resolution failures during month-end close are not an option."

## The Next PAIN

ESM works. TypeScript compiles. Tests run. But your journal entries are still in-memory arrays. Your `.env` file has no database URL. You deploy to a cloud instance. The filesystem is ephemeral. Your ledger vanishes on every deploy. Auditors require persistence.
