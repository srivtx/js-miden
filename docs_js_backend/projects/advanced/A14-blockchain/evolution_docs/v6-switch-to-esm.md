...
```typescript
// src/routes/transaction.ts (CommonJS style)
...
## ESM Evolution in the Blockchain
...
>
> You: "Because ESM is browser-native, and browsers don't know about `.ts`. TypeScript is a compile-time fiction. The runtime sees `.js`. Write what the runtime sees. In a blockchain, module resolution failures during consensus are not an option."

## The Next PAIN

ESM works. TypeScript compiles. Tests run. But your transactions are still in-memory arrays. Your `.env` file has no database URL. You deploy to a cloud instance. The filesystem is ephemeral. Your ledger vanishes on every deploy.
