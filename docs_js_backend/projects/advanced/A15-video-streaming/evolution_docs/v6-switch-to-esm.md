# v6 — Switch to ESM (Video Streaming)
...
```typescript
// src/routes/stream.routes.ts (CommonJS style)
...
import type { Video, StreamVariant } from '../types/stream.types.js';
```
...
import { streamService } from '../services/stream.service.js'; // ESM
...
## ESM Evolution in Video Streaming
...
>
> You: "Because ESM is browser-native, and browsers don't know about `.ts`. TypeScript is a compile-time fiction. The runtime sees `.js`. Write what the runtime sees. In a video streaming platform, module resolution failures during a live event are not an option."

## The Next PAIN

ESM works. TypeScript compiles. Tests run. But your video files are stored on local disk. Your `.env` file has no CDN URL. You deploy to a cloud instance. The filesystem is ephemeral. Your videos vanish on every deploy.
