...
```typescript
// src/routes/game.ts (CommonJS style)
...
import { updateStateSchema } from '../schemas/game.js';
...
## ESM Evolution in the Game Server
...
>
> You: "Because ESM is browser-native, and browsers don't know about `.ts`. TypeScript is a compile-time fiction. The runtime sees `.js`. Write what the runtime sees. In a game server, module resolution failures during a tournament are not an option."

## The Next PAIN

ESM works. TypeScript compiles. Tests run. But your game sessions are still in-memory Maps. Your `.env` file has no database URL. You deploy to a cloud instance. The filesystem is ephemeral. Your leaderboards vanish on every deploy.
