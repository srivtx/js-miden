# v6 — Switch to ESM (Healthcare FHIR)
...
```typescript
// src/routes/patient.ts (CommonJS style)
...
import type { Patient, AuditEvent } from '../types.js';
```
...
## ESM Evolution in Healthcare FHIR
...
>
> You: "Because ESM is browser-native, and browsers don't know about `.ts`. TypeScript is a compile-time fiction. The runtime sees `.js`. Write what the runtime sees. In a healthcare API, module resolution failures during a hospital go-live are not an option."

## The Next PAIN

ESM works. TypeScript compiles. Tests run. But your patient records are still in-memory arrays. Your `.env` file has no database URL. You deploy to a cloud instance. The filesystem is ephemeral. Your PHI vanishes on every deploy. HIPAA requires persistence.
