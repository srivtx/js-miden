...
5. **Test runner config**: Jest/Vitest need different settings for ESM. Mocks work differently.
...
> 
> You: "Because ESM is browser-native, and browsers don't know about `.ts`. TypeScript is a compile-time fiction. The runtime sees `.js`. Write what the runtime sees."

## The Next PAIN

ESM works. TypeScript compiles. Tests run. But your Prisma schema points to a local SQLite file. Your `.env` file has `DATABASE_URL="file:./dev.db"`. You deploy to Render/Heroku. The filesystem is ephemeral. Your database resets on every deploy.
