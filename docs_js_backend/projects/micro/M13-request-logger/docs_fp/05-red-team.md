# M13 Request Logger: Red Team

## Attack Scenarios

Your logger is not a security boundary — until it becomes one. Here is how an attacker weaponizes your logs.
...
Your logs grow at 100,000 lines per second. If each line is 200 bytes (because you log the full body), that is 20 MB/s. In 24 hours, that is 1.7 TB. Your disk fills. Your log aggregator costs explode. Your retention policy forces deletion of older logs — including the evidence of the attacker's earlier, more subtle intrusion.
...
4. **Body size limits**: Never log bodies larger than 10 KB. Truncate or omit them.
...
2. **Redact known sensitive fields recursively**:
   ```typescript
...
| Sensitive Data Leak | Credential exposure, regulatory fine | Recursive redaction, allowlists, encryption, IAM |
