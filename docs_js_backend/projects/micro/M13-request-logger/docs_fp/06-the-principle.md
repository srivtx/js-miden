# M13 Request Logger: The Principle
...
- An unredacted entry is a breach.

If you cannot audit your system from its logs alone, your logs are inadequate. If you can audit your system but the audit reveals plaintext passwords, your logs are toxic.

## The Principle

> **Log the contract, not the implementation. Redact the sensitive, not the convenient.**
>
...
>
> A password is never part of the contract. It is a secret shared between the user and the authentication system. If it appears in a log, the contract is broken.

A health check should not log `200 OK` every 2 seconds. It should log the moment it transitions from healthy to unhealthy, and the moment it recovers. The steady state is silent. The change is signal. The secret is redacted.
...
4. If a hacker reads this line, what do they learn?

If the answer to the first three is "no," delete the line. If the answer to the fourth is "passwords," delete the line and fix the system. Your future self — exhausted, paged, and desperate for signal — will thank you. Your security team will too.
